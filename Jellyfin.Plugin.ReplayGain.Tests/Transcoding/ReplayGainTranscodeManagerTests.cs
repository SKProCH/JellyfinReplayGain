...
public sealed class ReplayGainTranscodeManagerTests
{
    ...
    public async Task StartFfMpeg_WhenDisabled_PassesOriginalCommand()
    {
        ...
        await manager.StartFfMpeg(state, "output.m4a", "-i input.flac -codec:a aac", Guid.Empty, TranscodingJobType.Progressive, cancellation);
    ...
    private static Mock<ITranscodeManager> CreateInner(out List<string> receivedCommands)
    {
        ...
        inner
            .Setup(manager => manager.StartFfMpeg(
                ...
                It.IsAny<string?>()))
            .Callback<StreamState, string, string, Guid, TranscodingJobType, CancellationTokenSource, string?>(
                (_, _, command, _, _, _, _) => commands.Add(command))
