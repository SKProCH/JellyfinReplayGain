...
public sealed class LoudnormAnalyzer : IScheduledTask
{
    ...
    public string Name => "ReplayGain loudnorm analysis";

    public string Description => "Analyzes audio streams for two-pass loudnorm playback.";

    public string Category => "Library";

    public string Key => "ReplayGainLoudnormAnalysis";
    ...
    public async Task ExecuteAsync(IProgress<double> progress, CancellationToken cancellationToken)
    {
        ...
        var items = _libraryManager.GetItemList(new InternalItemsQuery
        {
            ...
            Recursive = true
        }).Where(item => item.IsFileProtocol && File.Exists(item.Path)).ToArray();
    ...
    private async Task AnalyzeIfNeededAsync(BaseItem item, CancellationToken cancellationToken)
    {
        ...
        var audioStreams = item.GetMediaStreams().Where(stream => stream.Type == MediaStreamType.Audio).ToArray();
        ...
        var filters = audioStreams.Select((_, index) =>
            $"[0:a:{index}]loudnorm=I={Format(config.LoudnormIntegratedLoudness)}:TP={Format(config.LoudnormTruePeak)}:LRA={Format(config.LoudnormLoudnessRange)}:print_format=json[norm{index}]");
